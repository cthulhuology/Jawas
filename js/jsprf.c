...
*/
static int
cvt_ll(SprintfState * ss, JSInt64 num, int width, int prec, int radix,
       int type, int flags, const char *hexp)
{
	char            cvtbuf[100];
	char           *cvt;
...
*/
static int
cvt_s(SprintfState * ss, const char *s, int width, int prec,
      int flags)
{
	int             slen;
...
static int
cvt_ws(SprintfState * ss, const jschar * ws, int width, int prec,
       int flags)
{
	int             result;
	/*
	 * Supply NULL as the JSContext; errors are not reported, and malloc() is used to allocate the buffer buffer.
	 */
	if (ws) {
...
BuildArgArray(const char *fmt, va_list ap, int *rv, struct NumArgState * nasArray)
{
	int             number = 0, cn = 0, i;
	const char     *p;
	char            c;
...
dosprintf(SprintfState * ss, const char *fmt, va_list ap)
{
	...
	fetch_and_convert:
		...
		case 's':
			if (type == TYPE_INT16) {
				/*
				 * This would do a simple string/byte conversion if JS_C_STRINGS_ARE_UTF8 is not defined.
				 */
...
static int
FuncStuff(SprintfState * ss, const char *sp, JSUint32 len)
{
	int             rv;
...
*/
static int
GrowStuff(SprintfState * ss, const char *sp, JSUint32 len)
{
	ptrdiff_t       off;
	char           *newbase;
	JSUint32        newlen;
...
*/
static int
LimitStuff(SprintfState * ss, const char *sp, JSUint32 len)
{
