...
/*
 * Please send bug reports to David M. Gay Bell Laboratories, Room 2C-463 600 Mountain Avenue Murray Hill, NJ 07974-0636 U.S.A. dmg@bell-labs.com
 */

/*
 * On a machine with IEEE extended-precision registers, it is necessary to specify double-precision (53-bit) rounding precision before invoking strtod or dtoa.  If the machine uses (the equivalent of) Intel 80x87
 * arithmetic, the call _control87(PC_53, MCW_PC); does this with many compilers.  Whether this or another call is appropriate depends on the compiler; for this to work, it may be necessary to #include "float.h" or
 * another system-dependent header file.
 */

/*
 * strtod for IEEE-arithmetic machines.
 *
 * This strtod returns a nearest machine number to the input decimal string (or sets err to JS_DTOA_ERANGE or JS_DTOA_ENOMEM).  With IEEE arithmetic, ties are broken by the IEEE round-even rule.  Otherwise ties are
 * broken by biased rounding (add half and chop).
 *
 * Inspired loosely by William D. Clinger's paper "How to Read Floating Point Numbers Accurately" [Proc. ACM SIGPLAN '90, pp. 92-101].
 *
 * Modifications:
 *
 * 1. We only require IEEE double-precision arithmetic (not IEEE double-extended). 2. We get by with floating-point arithmetic in a case that Clinger missed -- when we're computing d * 10^n for a small integer d and
 * the integer n is not too much larger than 22 (the maximum integer k for which we can represent 10^k exactly), we may be able to compute (d*10^k) * 10^(e-k) with just one roundoff. 3. Rather than a bit-at-a-time
 * adjustment of the binary result in the hard case, we use floating-point arithmetic to determine the adjustment to within one bit; only in really hard cases do we need to compute a second residual. 4. Because of
 * 3., we don't need a large table of powers of 10 for ten-to-e (just some small tables, e.g. of 10^k for 0 <= k <= 22).
 */
...
/*
 * Out-of-memory testing.  Use a good testcase (over and over) and then use these routines to cause a memory failure on every possible Balloc allocation, to make sure that all out-of-memory paths can be followed.
 * See bug 14044.
 */
...
multadd(Bigint * b, int32 m, int32 a)
{
	...
	if (++allocationNum == desiredFailure) {
		/*
		 * Faux allocation, because I'm not getting all of the failure paths without it.
		 */
		printf("Forced Failing Allocation number %d\n", allocationNum);
		Bfree(b);
		return NULL;
...
static Bigint  *
mult(CONST Bigint * a, CONST Bigint * b)
{
	CONST Bigint   *t;
	Bigint         *c;
	int32           k, wa, wb, wc;
 ...
 */
static Bigint  *
d2b(double d, int32 * e, int32 * bits)
{
	Bigint         *b;
	int32           de, i, k;
	ULong          *x, y, z;
...
{
	int32           scale;
	int32           bb2, bb5, bbe, bd2, bd5, bbbits, bs2, c, dsign, e, e1, esign, i, j, k, nd, nd0, nf, nz, nz0, sign;
	CONST char     *s, *s0, *s1;
...
dig_done:
	...
	/*
	 * Now we have nd0 digits, starting at s0, followed by a decimal point, followed by nd-nd0 digits.  The number we're after is the integer represented by those digits times 10**e
	 */
...
#else
			/*
			 * Compute adj so that the IEEE rounding rules will correctly round rv + adj in some half-way cases. If rv * ulp(rv) is denormalized (i.e., y <= (P-1)*Exp_msk1), we must adjust aadj to avoid
...
/*
 * Return floor(b/S) and set b to be the remainder.  As added restrictions, b must not have more words than S, the most significant word of S must not start with a 1 bit, and the returned quotient must be less than
 * 36.
 */
static          int32
 ...
 *
 * Inspired by "How to Print Floating-Point Numbers Accurately" by Guy L. Steele, Jr. and Jon L. White [Proc. ACM SIGPLAN '90, pp. 92-101].
 *
 * Modifications: 1. Rather than iterating, we use a simple numeric overestimate to determine k = floor(log10(d)).  We scale relevant quantities using O(log2(k)) rather than O(k) multiplications. 2. For some modes > 2
 * (corresponding to ecvt and fcvt), we don't try to generate digits strictly left to right.  Instead, we compute with fewer bits and propagate the carry if necessary when rounding the final digit up.  This is
 * often faster. 3. Under the assumption that input will be rounded nearest, mode 0 renders 1e23 as 1e23 rather than 9.999999999999999e22. That is, we allow equality in stopping tests when the round-nearest rule
 * will give the same floating-point value as would satisfaction of the stopping test with strict inequality. 4. We remove common factors of powers of 2 from relevant quantities. 5. When converting floating-point
 * integers less than 1e16, we use floating-point arithmetic rather than resorting to multiple-precision integers. 6. When asked to produce fewer than 15 digits, we first try to get by with floating-point
 * arithmetic; we resort to multiple-precision integer arithmetic only if we cannot guarantee that the floating-point calculation has given the correctly rounded result.  For k requested digits and "uniformly"
 * distributed input, the probability is something like 10^(k-15) that we must resort to the Long calculation.
 */

/* Always emits at least one digit. */
/*
 * If biasUp is set, then rounding in modes 2 and 3 will round away from zero when the number is exactly halfway between two representable values.  For example, rounding 2.5 to zero digits after the decimal point
 * will return 3 and not 2. 2.49 will still round to 2, and 2.51 will still round to 3.
 */
/*
 * bufsize should be at least 20 for modes 0 and 1.  For the other modes, bufsize should be two greater than the maximum number of output characters expected.
 */
static          JSBool
...
{
	/*
	 * Arguments ndigits, decpt, sign are similar to those of ecvt and fcvt; trailing zeros are suppressed from the returned string.  If not null, *rve is set to point to the end of the return value.  If d is
	 * +-Infinity or NaN, then *decpt is set to 9999.
	 *
	 * mode: 0 ==> shortest string that yields d when read in and rounded to nearest. 1 ==> like 0, but with Steele & White stopping rule; e.g. with IEEE P754 arithmetic , mode 0 gives 1e23 whereas mode 1 gives
	 * 9.999999999999999e22. 2 ==> max(1,ndigits) significant digits.  This gives a return value similar to that of ecvt, except that trailing zeros are suppressed. 3 ==> through ndigits past the decimal point.
	 * This gives a return value similar to that from fcvt, except that trailing zeros are suppressed, and ndigits can be negative. 4-9 should give the same return values as 2-3, i.e., 4 <= mode <= 9 ==> same
	 * return as mode 2 + (mode & 1).  These modes are mainly for debugging; often they run slower but sometimes faster than modes 2-3. 4,5,8,9 ==> left-to-right digit generation. 6-9 ==> don't try fast
	...
	int32           bbits, b2, b5, be, dig, i, ieps, ilim, ilim0, ilim1, j, j1, k, k0, k_check, leftright, m2, m5, s2, s5, spec_case, try_quick;
	Long            L;
#ifndef Sudden_Underflow
	int32           denorm;
	ULong           x;
...
no_digits:
		*decpt = 1;
		...
		if (rve)
			*rve = buf + 1;
		/*
		 * We might have jumped to "no_digits" from below, so we need to be sure to free the potentially allocated Bigints to avoid memory leaks.
		 */
		Bfree(b);
		Bfree(S);
		if (mlo != mhi)
	...
	if ((i = (int32) (word0(d) >> Exp_shift1 & (Exp_mask >> Exp_shift1))) != 0) {
		 ...
		 *
		 * We want k to be too large rather than too small. The error in the first-order Taylor series approximation is in our favor, so we just round up the constant enough to compensate for any error in the
		 * multiplication of (i - Bias) by 0.301029995663981; since |i - Bias| <= 1077, and 1077 * 0.30103 * 2^-52 ~=~ 7.2e-14, adding 1e-13 to the constant term more than suffices. Hence we adjust the
		 * constant term to 0.1760912590558. (We could get a more accurate k by invoking log10, but this is probably not worthwhile.)
		 */
	...
	/*
	 * At this point floor(log10(d)) <= k <= floor(log10(d))+1. If k_check is zero, we're guaranteed that k = floor(log10(d)).
	 */
	j = bbits - i - 1;
	...
	/*
	 * At this point d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5), where b is an odd integer, b2 >= 0, b5 >= 0, s2 >= 0, and s5 >= 0.
	 */
	if (mode < 0 || mode > 9)
		mode = 0;
	try_quick = 1;
	...
	/*
	 * ilim1 is the maximum number of significant digits we want, based on k and ndigits, when it turns out that k was computed too high by one.
	 */
...
fast_failed:
	...
	if (leftright) {
		if (mode < 2) {
			i =
				...
				1 + P - bbits;
			/*
			 * i is 1 plus the number of trailing zero bits in d's significand. Thus, (2^m2 * 5^m5) / (2^(s2+i) * 5^s5) = (1/2 lsb of d)/10^k.
			 */
		} else {
			j = ilim - 1;
			if (m5 >= j)
		...
		if (!mhi)
			goto nomem;
		/*
		 * (mhi * 2^m2 * 5^m5) / (2^s2 * 5^s5) = one-half of last printed (when mode >= 2) or input (when mode < 2) significant digit, divided by 10^k.
		 */
	}
	/*
	 * We still have d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5).  Reduce common factors in b2, m2, and s2 without changing the equalities.
	 */
	if (m2 > 0 && s2 > 0) {
		i = m2 < s2 ? m2 : s2;
		b2 -= i;
	...
	/*
	 * Now we have d/10^k = (b * 2^b2) / (2^s2 * 5^s5) and (mhi * 2^m2) / (2^s2 * 5^s5) = one-half of last printed or input significant digit, divided by 10^k.
	 */
	...
	/*
	 * Now we have d/10^k = (b * 2^b2) / (S * 2^s2) and (mhi * 2^m2) / (S * 2^s2) = one-half of last printed or input significant digit, divided by 10^k.
	 */
	...
	if (mode < 2) {
		if (!word1(d) && !(word0(d) & Bndry_mask)
			...
			/*
			 * The special case.  Here we want to be within a quarter of the last input significant digit instead of one half of it when the decimal output string's value is less than d.
			 */
			b2 += Log2P;
			s2 += Log2P;
			spec_case = 1;
	...
	/*
	 * Now we have d/10^k = b/S and (mhi * 2^m2) / S = maximum acceptable error, divided by 10^k.
	 */
	if (k_check) {
		if (cmp(b, S) < 0) {
			k--;
	...
	if (ilim <= 0 && mode > 2) {
		/*
		 * We're doing fixed-mode output and d is less than the minimum nonzero output in this mode. Output either zero or the minimum nonzero output depending on which is closer to d.
		 */
		if (ilim < 0)
			goto no_digits;
		S = multadd(S, 5, 0);
		...
		if (i < 0 || (i == 0 && !biasUp)) {
			/*
			 * Always emit at least one digit.  If the number appears to be zero using the current mode, then emit one '0' digit and set decpt to 1.
			 */
			/*
	...
	if (leftright) {
		...
		for (i = 1;; i++) {
			...
			if ((j < 0) || (j == 0 && !mode
				...
				if (j1 > 0) {
					/*
					 * Either dig or dig+1 would work here as the least significant decimal digit. Use whichever would produce a decimal value closer to d.
					 */
					b = lshift(b, 1);
					if (!b)
						goto nomem;
...
/*
 * Let b = floor(b / divisor), and return the remainder.  b must be nonnegative. divisor must be between 1 and 65536. This function cannot run out of memory.
 */
static          uint32
...
/*
 * "-0.0000...(1073 zeros after decimal point)...0001\0" is the longest string that we could produce, which occurs when printing -5e-324 in binary.  We could compute a better estimate of the size of the output
	...
	if (buffer) {
		...
		nomem2:
			...
			if (!s)
				goto nomem2;
			/*
			 * At this point we have the following: s = 2^s2; 1 > df = b/2^s2 > 0; (d - prevDouble(d))/2 = mlo/2^s2; (nextDouble(d) - d)/2 = mhi/2^s2.
			 */
