...
typedef struct EmitStateStackEntry {
	...
	JSPackedBool    jumpToJumpFlag;	/* true if we've patched jump-to-jump to avoid 16-bit unsigned offset overflow */
}               EmitStateStackEntry;
...
typedef struct REMatchState {
	const jschar   *cp;
	RECapture       parens[1];	/* first of 're->parenCount' captures, allocated at end of this struct */
}               REMatchState;
...
typedef struct REGlobalData {
	...
	JSArenaPool     pool;	/* It's faster to use one malloc'd pool than to malloc/free the three items that are allocated from this pool */
}               REGlobalData;
	  ...
	  intN operandSP)
{
	...
	case REOP_ALT:
		...
		if (((RENode *) result->kid)->op == REOP_FLAT &&
			...
			/*
			 * ALTPREREQ, <end>, uch1, uch2, <next>, ..., JUMP, <end> ... ENDALT
			 */
			state->progLength += 13;
			...
			/*
			 * ALTPREREQ2, <end>, uch1, uch2, <next>, ..., JUMP, <end> ... ENDALT
			 */
			state->progLength += 13;
