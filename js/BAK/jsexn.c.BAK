		...
		if (fp->fun) {
			...
			for (i = 0; i < fp->argc; i++) {
				...
				if (JSVAL_IS_PRIMITIVE(v)) {
					...
					/*
					 * XXX Avoid toString on objects, it takes too long and uses too much memory, for too many classes (see Mozilla bug 166743).
					 */
					char            buf[100];
...
/*
 * XXXbe Consolidate the ugly truth that we don't treat filename as UTF-8 with these two functions.
 */
static JSString *
