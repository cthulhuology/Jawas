...
{
	int             sizeof_char, sizeof_short, sizeof_int, sizeof_int64, sizeof_long, sizeof_float, sizeof_double, sizeof_word, sizeof_dword;
	int             bits_per_int64_log2, align_of_short, align_of_int, align_of_long, align_of_int64, align_of_float, align_of_double, align_of_pointer, align_of_word;
	int             dummy1;
...
#error "Endianess not defined."
         ...
         */
	{
		...
		if (sizeof(short) == 2) {
			/*
			 * force |volatile| here to get rid of any compiler optimisations (var in register etc.) which may be appiled to |auto| vars - even those in |union|s... (|static| is used to get the same
			 * functionality for compilers which do not honor |volatile|...).
			 */
