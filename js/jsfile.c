...
typedef struct JSFile {
	...
	JSBool          hasRandomAccess;	/* can the file be randomly accessed? false for stdin, and UTF-encoded files. */
	JSBool          append;
	JSBool          truncate;
 ...
 *
 * Function takes one UCS-2 char and writes it to a UTF-8 buffer. We need a UTF-8 buffer because we don't know before this function how many bytes of utf-8 data will be written. It also takes a pointer to the end of
 * the UTF-8 buffer so that we don't overwrite data. This function returns the number of UTF-8 bytes of data written, or -1 if the buffer would have been overrun.
 */
 ...
 *
 * strcasecmp is in strings.h, but on windows it's called _stricmp... will need to #ifdef this
 */
...
js_FileSeek(JSContext * cx, JSFile * file, int32 len, int32 mode)
{
	...
	switch (mode) {
	case ASCII:
		count = lseek(file->handle, len, SEEK_CUR);
		break;
	...
	case UCS2:
		count = lseek(file->handle, len * 2, SEEK_CUR) / 2;
		break;
...
js_FileWrite(JSContext * cx, JSFile * file, jschar * buf, int32 len, int32 mode)
{
	...
	case ASCII:
		...
		count = write(file->handle, aux, len);
	...
	case UCS2:
		count = write(file->handle, buf, len * 2) >> 1;
...
js_size(JSContext * cx, JSFile * file)
{
	...
	if (stat(file->path, &st))
		return JSVAL_VOID;
	return INT_TO_JSVAL(st.st_size);
}
...
int
system_command(JSContext * cx, char *command_line, const char *env[], int *in, int *out)
{
	int             pid;
	int             pipe_stdin[2];
	int             pipe_stdout[2];
	...
	if (pipe(pipe_stdin) || pipe(pipe_stdout)) {
		JS_ReportWarning(cx, "Failed to open pipes");
		return -1;
	}
	if ((pid = fork()) < 0) {
		JS_ReportWarning(cx, "Failed to fork child");
		return -1;
	}
	if (pid == 0) {
		...
		errno = 0;
		execle("/bin/sh", "sh", "-c", command_line, NULL, env);
		if (errno) {
			JS_ReportWarning(cx, strerror(errno));
		}
		exit(0);
	} else {
		if (in) {
...
file_open(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rval)
{
	...
	mask |= js_FileHasOption(cx, mode, "read") ? O_RDONLY : 0;
	mask |= js_FileHasOption(cx, mode, "write") ? O_WRONLY : 0;
	mask |= js_FileHasOption(cx, mode, "readWrite") ? O_RDWR : 0;
	mask |= js_FileHasOption(cx, mode, "append") ? O_APPEND : 0;
	mask |= js_FileHasOption(cx, mode, "create") ? O_CREAT : 0;
	mask |= js_FileHasOption(cx, mode, "replace") ? O_TRUNC : 0;
	...
	if (file->path[0] == PIPE_SYMBOL || file->path[len - 1] == PIPE_SYMBOL) {
		if (file->path[0] == PIPE_SYMBOL && file->path[len - 1] == PIPE_SYMBOL) {
			...
			int             i = 0;
			if (file->path[0] == PIPE_SYMBOL) {
				if (mask & (O_WRONLY | O_APPEND | O_CREAT | O_TRUNC)) {
					JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
							     JSFILEMSG_OPEN_MODE_NOT_SUPPORTED_WITH_PIPES,
							     mode, file->path);
					goto out;
				}
				system_command(cx, &file->path[1], NULL, &file->handle, NULL);
			} else if (file->path[len - 1] == PIPE_SYMBOL) {
				char           *command = JS_malloc(cx, len);

				strncpy(command, file->path, len - 1);
				command[len - 1] = '\0';
				system_command(cx, &file->path[1], NULL, NULL, &file->handle);
				JS_free(cx, command);
			}
			/* set the flags */
			file->isPipe = JS_TRUE;
		...
		file->handle = open(file->path, mask, 0644);
	}
...
file_remove(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rval)
{
	...
	if ((js_isDirectory(cx, file) ?
	     rmdir(file->path) : unlink(file->path)) == 0) {
		js_ResetAttributes(file);
		*rval = JSVAL_TRUE;
		return JS_TRUE;
	} else {
...
file_copyTo(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rval)
{
	...
	handle = open(dest, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	...
	count = INT_TO_JSVAL(read(file->handle, buffer, size));
	...
	count = INT_TO_JSVAL(write(handle, buffer, JSVAL_TO_INT(size)));
...
JSObject       *
js_NewFileObjectFromFILE(JSContext * cx, int handle, char *filename, int32 mode, JSBool open, JSBool randomAccess)
{
	JSObject       *obj;
	JSFile         *file;
...
file_getProperty(JSContext * cx, JSObject * obj, jsval id, jsval * vp)
{
	...
	case FILE_MODE:
		...
		if ((file->mode & O_WRONLY) == O_WRONLY) {
			if (flag)
				strcat(bytes, ",");
			strcat(bytes, "write");
			flag = JS_TRUE;
		}
		if ((file->mode & O_RDWR) == O_RDWR) {
			if (flag)
				strcat(bytes, ",");
			strcat(bytes, "readWrite");
			flag = JS_TRUE;
		}
		if ((file->mode & O_APPEND) == O_APPEND) {
			if (flag)
				strcat(bytes, ",");
			strcat(bytes, "append");
			flag = JS_TRUE;
		}
		if ((file->mode & O_CREAT) == O_CREAT) {
			if (flag)
				strcat(bytes, ",");
			strcat(bytes, "create");
			flag = JS_TRUE;
		}
		if ((file->mode & O_TRUNC) == O_TRUNC) {
			if (flag)
				strcat(bytes, ",");
	...
	case FILE_POSITION:
		...
		if (file->isOpen && js_isFile(cx, file)) {
			int pos = lseek(file->handle, 0, SEEK_CUR);
			if (pos != -1) {
				*vp = INT_TO_JSVAL(pos);
			} else {
				JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
...
file_setProperty(JSContext * cx, JSObject * obj, jsval id, jsval * vp)
{
	...
	case FILE_POSITION:
		...
		if (file->isOpen && js_isFile(cx, file)) {
			...
			pos = lseek(file->handle, offset, SEEK_SET);
...
js_InitFileClass(JSContext * cx, JSObject * obj)
{
	...
	/*
	 * Define CURRENTDIR property. We are doing this to get a slash at the end of the current dir
	 */
