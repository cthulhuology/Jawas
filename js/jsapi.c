...
JS_YieldRequest(JSContext * cx)
{
	...
	JS_UNLOCK_GC(rt);
	/*
	 * XXXbe give the GC or another request calling it a chance to run here? Assumes FIFO scheduling
	 */
	JS_LOCK_GC(rt);
	rt->requestCount++;
	JS_UNLOCK_GC(rt);
	     ...
	     JSPropertySpec * static_ps, JSFunctionSpec * static_fs)
{
	...
	if (!constructor) {
		...
		if (OBJ_GET_CLASS(cx, ctor) == clasp) {
			/*
			 * XXXMLM - this fails in framesets that are writing over themselves! JS_ASSERT(!OBJ_GET_PROTO(cx, ctor));
			 */
...
int             CALLBACK
LibMain(HINSTANCE hInst, WORD wDataSeg,
