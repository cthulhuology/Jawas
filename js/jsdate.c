...
/*
 * Work around msvc double optimization bug by making these runtime values; if they're available at compile time, msvc optimizes division by them by computing the reciprocal and multiplying instead of dividing -
 * this loses when the reciprocal isn't representable in a double.
 */
 ...
 *
 * yearStartingWith[0][i] is an example non-leap year where Jan 1 appears on Sunday (i == 0), Monday (i == 1), etc.
 *
 * yearStartingWith[1][i] is an example leap year where Jan 1 appears on Sunday (i == 0), Monday (i == 1), etc.
 */
 ...
 */
/*
 * XXX this function must be above date_parseString to avoid a horrid bug in the Win16 1.52 compiler
 */
#define MAXARGS        7
static          JSBool
date_UTC(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rval)
{
	...
	/*
	 * if we got a 0 for 'date' (which is out of range) pretend it's a 1.  (So Date.UTC(1972, 5) works)
	 */
	if (array[2] < 1)
		array[2] = 1;
...
date_parseString(JSString * str, jsdouble * result)
{
	...
	while (i < limit) {
		...
		if ('0' <= c && c <= '9') {
			...
			/*
			 * uses of seenplusminus allow : in TZA, so Java no-timezone style of GMT+4:30 works
			 */
			...
			if ((prevc == '+' || prevc == '-') /* && year>=0 */ ) {
				...
				/*
				 * until it is determined that mon is the actual month, keep it as 1-based rather than 0-based
				 */
				if (mon < 0)
					mon = /* byte */ n;
				else if (mday < 0)
				...
				if (date_regionMatches(wtb[k], 0, s, st, i - st, 1)) {
					...
					if (action != 0) {
						if (action < 0) {
							...
							/*
							 * Adjust mon to be 1-based until the final values for mon, mday and year are adjusted below
							 */
							if (seenmonthname) {
								goto syntax;
							}
	      ...
	      uintN maxargs, JSBool local, jsval * rval)
{
	...
	/*
	 * Satisfy the ECMA rule that if a function is called with fewer arguments than the specified formal arguments, the remaining arguments are set to undefined.  Seems like all the Date.setWhatever functions
	 * in ECMA are only varargs beyond the first argument; this should be set to undefined if it's not given.  This means that "d = new Date(); d.setMilliseconds()" returns NaN.  Blech.
	 */
	if (argc == 0)
	      ...
	      jsval * argv, uintN maxargs, JSBool local, jsval * rval)
{
	...
	/*
	 * return NaN if date is NaN and we're not setting the year, If we are, use 0 as the time.
	 */
	if (!(JSDOUBLE_IS_FINITE(result))) {
		if (argc < 3)
			return js_NewNumberValue(cx, result, rval);
		 ...
		 jsval * argv, jsval * rval)
{
	...
	if (!JSDOUBLE_IS_FINITE(*date)) {
		...
		/*
		 * Avoid dependence on PRMJ_FormatTimeUSEnglish, because it requires a PRMJTime... which only has 16-bit years.  Sub-ECMA.
		 */
...
/*
 * for Date.toLocaleString; interface to PRMJTime date struct. If findEquivalent is true, then try to map the year to an equivalent year that's in range.
 */
static void
new_explode(jsdouble timeval, PRMJTime * split, JSBool findEquivalent)
{
	...
	if (year > 32767 || year < -32768) {
		if (findEquivalent) {
			/*
			 * We're really just trying to get a timezone string; map the year to some equivalent year in the range 0 to 2800.  Borrowed from A. D. Olsen.
			 */
			jsint           cycles;
#define CYCLE_YEARS 2800L
	...
	/*
	 * not sure how this affects things, but it doesn't seem to matter.
	 */
...
date_format(JSContext * cx, jsdouble date, formatspec format, jsval * rval)
{
	...
	if (!JSDOUBLE_IS_FINITE(date)) {
		...
		/*
		 * offset from GMT in minutes.  The offset includes daylight savings, if it applies.
		 */
		...
		/*
		 * print as "Wed Nov 05 19:38:03 GMT-0800 (PST) 1997" The TZA is printed as 'GMT-0800' rather than as 'PST' to avoid operating-system dependence on strftime (which PRMJ_FormatTimeUSEnglish calls,
		 * for %Z only.)  win32 prints PST as 'Pacific Standard Time.'  This way we always know what we're getting, and can parse it if we produce it. The OS TZA string is included as a comment.
		 */

		/*
		 * get a timezone string from the OS to include as a comment.
		 */
		...
		if (PRMJ_FormatTime(tzbuf, sizeof tzbuf, "(%Z)", &split) != 0) {
			 ...
			 *
			 * Reject it if it contains any non-ASCII, non-alphanumeric characters.  It's then likely in some other character encoding, and we probably won't display it correctly.
			 */
			usetz = JS_TRUE;
			tzlen = strlen(tzbuf);
			if (tzlen > 100) {
...
{
	/*
	 * Use '%#c' for windows, because '%c' is backward-compatible and non-y2k with msvc; '%#c' requests that a full year be used in the result string.
	 */
...
{
	/*
	 * Use '%#x' for windows, because '%x' is backward-compatible and non-y2k with msvc; '%#x' requests that a full year be used in the result string.
	 */
	return date_toLocaleHelper(cx, obj, argc, argv, rval,
#if defined(_WIN32) && !defined(__MWERKS__)
				   "%#x"
...
{
	/*
	 * It is an error to call date_valueOf on a non-date object, but we don't need to check for that explicitly here because every path calls date_getProlog, which does the check.
	 */
...
Date(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rval)
{
	...
	if (!(cx->fp->flags & JSFRAME_CONSTRUCTING)) {
		...
		/*
		 * NSPR 2.0 docs say 'We do not support PRMJ_NowMS and PRMJ_NowS', so compute ms from PRMJ_Now.
		 */
		us = PRMJ_Now();
		JSLL_UI2L(us2ms, PRMJ_USEC_PER_MSEC);
		JSLL_DIV(ms, us, us2ms);
		...
		for (loop = 0; loop < MAXARGS; loop++) {
			if (loop < argc) {
				if (!js_ValueToNumber(cx, argv[loop], &double_arg))
					return JS_FALSE;
				/*
				 * if any arg is NaN, make a NaN date object and return
				 */
