...
#endif
#ifdef XP_WIN
	JSInt64         s, us, win2un = JSLL_INIT(0x19DB1DE, 0xD53E8000), ten = JSLL_INIT(0, 10);
...
#ifdef XP_WIN
	/*
	 * The windows epoch is around 1600. The unix epoch is around 1970. win2un is the difference (in windows time units which are 10 times more precise than the JS time unit)
	 */
	GetSystemTimeAsFileTime(&time);
	/*
	 * Win9x gets confused at midnight http://support.microsoft.com/default.aspx?scid=KB;en-us;q224423 So if the low part (precision <8mins) is 0 then we get the time again.
	 */
...
PRMJ_FormatTime(char *buf, int buflen, char *fmt, PRMJTime * prtm)
{
	...
	/*
	 * Zero out the tm struct.  Linux, SunOS 4 struct tm has extra members int tm_gmtoff, char *tm_zone; when tm_zone is garbage, strftime gets confused and dumps core.  NSPR20 prtime.c attempts to fill these
	 * in by calling mktime on the partially filled struct, but this doesn't seem to work as well; the result string has "can't get timezone" for ECMA-valid years.  Might still make sense to use this, but find
	 * the range of years for which valid tz information exists, and map (per ECMA hint) from the given year into that range.
	 *
	 * N.B. This hasn't been tested with anything that actually _uses_ tm_gmtoff; zero might be the wrong thing to set it to if you really need to format a time.  This fix is for jsdate.c, which only uses
	...
	/*
	 * Even with the above, SunOS 4 seems to detonate if tm_zone and tm_gmtoff are null.  This doesn't quite work, though - the timezone is off by tzoff + dst.  (And mktime seems to return -1 for the exact dst
	 * changeover time.)
	 *
	 */
	...
	if (mktime(&a) == -1) {
		/*
		 * Seems to fail whenever the requested date is outside of the 32-bit UNIX epoch.  We could proceed at this point (setting a.tm_zone to "") but then strftime returns a string with a 2-digit field of
		 * garbage for the year.  So we return 0 and hope jsdate.c will fall back on toString.
		 */
		return 0;
	}
#endif
