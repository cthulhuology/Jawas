...
EvalLuaFile(File fc)
{
	...
	for (int i = 0; fc->parsed[i].kind; ++i) {
		...
		case TEXT:
			write_chunk(client.request->socket,&fc->data[fc->parsed[i].offset],fc->parsed[i].length);
			break;
		case SCRIPT:
		case EMIT:
			if (luaL_loadbuffer(lins,&fc->data[fc->parsed[i].offset], fc->parsed[i].length, fc->name.data) || lua_pcall(lins,0,LUA_MULTRET,0)) error("Failed to evaluate [%s]",copy(&fc->data[fc->parsed[i].offset],fc->parsed[i].length));
		}
	}
}
...
PrintLua(lua_State* l)
{
	...
	for (int i = 1; i <= n; ++i) {
		str out =  lua2str(i);
		write_chunked_socket(client.request->socket,out);
	}
	lua_pop(l,n);
	return 0;
}
...
HeaderLua(lua_State* l)
{
	...
	str s = lua2str(1);
	lua_pop(l,n);
	str2lua(find_header(client.request->headers,s));
	return 1;
}
...
ParamLua(lua_State* l)
{
	...
	str s = lua2str(1);
	lua_pop(l,n);
	str2lua(find_header(client.request->query_vars,s));
	return 1;
}
...
PostHTTPLua(lua_State* l)
{
	...
	request_callback(req,client.response,callback);
	send_request(req);
	if (client.response) client.response->status = 0;
	lua_pop(l,n+1);
	longjmp(l_jump,1);
	return 0;
}
...
SocketLua(lua_State* l)
{
	...
	Socket s = create_socket(new_socket(0),NULL);
	s->host = host;
...
run_lua_script(File fc, Headers data)
{
	...
	if (!setjmp(l_jump)) {
		if (client.response) EvalLuaFile(fc);
		else if (luaL_loadbuffer(lins,fc->data, fc->st.st_size, fc->name.data) || lua_pcall(lins,0,LUA_MULTRET,0)) error("Failed to evaluate script %c",fc->name);
	}
	end_lua();
	return 0;
}
...
lws_handler(File fc)
{
	cache_control(client.response->headers,"max-age=0, no-cache");
